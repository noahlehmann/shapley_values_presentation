...
The concept of explaining a models way of working is called \ac{XAI}.
The key idea of this concept can be described as followed:
\begin{quotation}
    It is impossible to trust a machine learning model with-
    out understanding how and why it makes its decisions and
    whether these decisions are justified\cite{explainability}.
\end{quotation}

The key objective of \ac{XAI} therefore is to understand any model.
...
The later case is the exact use case for \emph{Shapley Values}.
...
Let's look at the following example in figure~\ref{fig:blackbox}, where - with a certainty of \num{90}\% - a model rated the sentence above as \emph{sad}.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.45\textwidth]{shap_1}
...
of 90\% certainty and thus gives the user the possibility to check on the models decision.
...
Why exactly this works will be shown in chapter~\ref{subsec:shap_math}.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.45\textwidth]{shap_2}
    \caption{Blackbox feature contribution view}
    \label{fig:comp}
\end{figure}
Shapley Values not only calculate a features - in the case of emotion analysis a word - contribution to the model's outcome
...
mathematical definition, before further examining their applications in \ac{XAI} and actual implementations.

Before looking at the complete formula for te calculation of Shapley Values, some variables need to be defined:
...
With the most important variables defined, let's look at the definition for Shapley Values:

\begin{figure}[h]
    \begin{displaymath}
        \Phi_i(v) = \sum_{S \subseteq N \setminus \{i\}}~\frac{|S|!~(n-|S|-1)!}{n!}~(v(S \cup \{i\})-v(S))
    \end{displaymath}
    \caption{Definition of Shapley Values}
\end{figure}
\label{math:shap}
\vspace{0.5cm}
...
output when applied to a subset of \(N\) excluding \(i\), \(S \subset N \setminus \{i\}\), it's weighted importance to other combinations
of \(S \subset N \setminus \{i\}\) and the sum of all these weighted importances for all possible subsets of \(N\) excluding \(i\),
...
including \(i\).
The difference in the outcomes of both function calls will then weight with the importance of the chosen subset \(S \subseteq N \setminus \{i\}\).
...
are multiplied and then weight with the total number of all possible subsets of \(N\).

As each weighted contribution of \(i\) to any subset \(S\) now only has the importance of the size of \(S\), the importances
of \(i\) need to be compared and summed over all possible variations of \(S \cup N \setminus \{i\}\).
...
After running the model on the sentence as a whole and the single words in \(N\), the following is observed:
\begin{itemize}
    \item \(v(\{h\}) = 0.5\)
    \item \(v(\{t\}) = 0.25\)
    \item \(v(\{h,t\}) = 1\)
...
\vspace{0.3cm}
\begin{displaymath}
    \Phi_h(v) = \sum_{S \subseteq \{\{t\},\emptyset\} \subseteq \{h,t\} \setminus \{h\}}~\frac{1}{2}~(v(S \cup \{i\})-v(S))
\end{displaymath}
\vspace{0.3cm}
\linebreak
...
\vspace{0.3cm}
\begin{displaymath}
    \Phi_t(v) = \sum_{S \subseteq \{\{h\},\emptyset\} \subseteq \{h,t\} \setminus \{t\}}~\frac{1}{2}~(v(S \cup \{i\})-v(S))
\end{displaymath}
\vspace{0.3cm}
\linebreak
...
In most use cases with large feature set, a preselection must be done before calculation the Shapley Values for the
selected features.
...
Approximations like \ac{LIME} can be used to reduce the models structural complexity, but again making the outcome an
approximation.
...
When applying approximation like \ac{LIME} as stated above, the subsets usually are not selected evenly, but rather
...
As the limitations above are only applicable, when the calculation of the Shapley Values surpass a computational limit, requiring
an approximation to reduce the complexity, they can be avoided by simple not approximating.
...
of \ac{XAI}.
Due to the complexity, the explanation of models is somewhat limited to the bulk-processing or continuous processing on the
server side.
A real time computation and explanation on a users enddevice is hardly doable and therefore not recommended.